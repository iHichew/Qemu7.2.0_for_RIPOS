...
extern uint64_t share_memory_address;
...
static const char *riscv_ram = "riscv_virt_board.ram";
    ...
    while (len > 0) {
        ...
        if (!(memory_region_is_ram(mr) ||
            ...
            case WRITE_DATA:
                memcpy(ram_ptr, buf, l);
                bool flag = true;
                const char *ram_name = "riscv_virt_board.ram";
                if(strcmp(mr->name,ram_name)==0 && flag == true){
                    //printf("[Qemu] image len is %x\n", l);
                    flag = false;
                    share_memory_address = ram_ptr;  
                }
                invalidate_and_set_dirty(mr, addr1, l);
                break;
            case FLUSH_CACHE:
