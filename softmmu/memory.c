...
// syscall number
const SYSCALL_OPENAT = 56;
const SYSCALL_READ = 63;
const SYSCALL_WRITE = 64;
const SYSCALL_EXIT = 93;
const SYSCALL_YIELD = 124;
const SYSCALL_GET_TIME = 169;
const SYSCALL_GETPID = 172;
const SYSCALL_FORK = 220;
const SYSCALL_EXEC = 221;
const SYSCALL_WAITPID = 260;
const SYSCALL_SYSINFO = 179;
const SYSCALL_SOCKET = 198;
const SYSCALL_BIND = 200;
const SYSCALL_LISTEN = 201;
const SYSCALL_ACCEPT = 202;
const SYSCALL_CONNECT = 203;
const SYSCALL_GETSOCKNAME = 204;
const SYSCALL_SEND = 206;
const SYSCALL_RECV = 207;
const SYSCALL_SETSOCKOPT = 208;
const SYSCALL_CLOSE = 209;
const SYSCALL_SOCK_SEND_BUFFER = 999;
                                                ...
                                                MemTxAttrs attrs)
{
    ...
    if(tmp == '#'){
        // printf("[In Qemu] share_memory_address: %lx\n",share_memory_address);
        uint64_t tohost_gpa = 0xD000000-0x200000;
        uintptr_t tohost_address = share_memory_address+tohost_gpa;
        uintptr_t fromhost_address = share_memory_address+tohost_gpa+0x8;
        uintptr_t magic_mem_address = *(uint64_t*)tohost_address;
        // printf("share_memory_address: %lx\n", share_memory_address);
        // printf("tohost_gpa: %lx\n", tohost_gpa);
        // printf("tohost_address: %lx\n", tohost_address);
        // printf("magic addr: %lx\n", magic_mem_address);

        uint64_t syscall_guest_num = *(uint64_t*)(tohost_address+magic_mem_address-(0x80200000+tohost_gpa));
        uint64_t syscall_guest_num_0 = *(uint64_t*)(tohost_address+magic_mem_address-(0x80200000+tohost_gpa));

        if (syscall_guest_num == 0xFFFFFFFF)
            return MEMTX_OK;

        uint64_t args[7];
        void* p_args[7];
        for (int i = 0; i < 7; i++) {
            args[i] = *(uint64_t*)(tohost_address + magic_mem_address - (0x80200000 + tohost_gpa) + 0x8 * (i + 1));
            p_args[i] = (void*)(tohost_address + args[i] - (0x80200000 + tohost_gpa));
        }

        // printf("[In Qemu] syscall %d: [", syscall_guest_num);
        // for (int i = 0; i < 5; i++) {
        //     i && printf(", ");
        //     printf("%llx", args[i]);
        // }
        ...
        switch (syscall_guest_num) {
        case 56:  // openat
            printf("[Qemu] filename: \"%s\"\n", (char*)p_args[1]);
            ret = openat(args[0] /* dirfd */, p_args[1] /* filename */, args[2] /* flag */, args[3] /* mode */);
            break;
        case 61:  // getdents
            ret = getdents64(args[0] /* fd */, p_args[1] /* dirp */, args[2] /* count */);
            break;
        case 48:  // faccessat
            // printf("[In Qemu] filename: \"%s\"\n", (char*)p_args[1]);
            ret = faccessat(args[0] /* dirfd */, p_args[1] /* filename */, args[2] /* mode */, args[3] /* flags */);
            break;
        case 57:  // close
            ret = close(args[0]);
            break;
        case 23:  // dup
            ...
            break;
        case 24:  // dup3
            ret = dup3(args[0] /* oldfd */, args[1] /* newfd */, args[2] /* flags */);
            break;
        case 43:  // statfs
            ret = statfs((char*)args[0] /* path */, (struct statfs*)args[1] /* statfs_s */);
            break;
        case 17: {  // getcwd
            char *addr = getcwd(p_args[0], args[1] /* size */);
            ret = strlen(addr);
            break;
        }
        case 46:  // ftruncate
            ret = ftruncate(args[0] /* fd */, args[1] /* length */);
            break;
        case 62:  // lseek
            ret = lseek(args[0] /* fd */, args[1] /* offset */, args[2] /* whence */);
            break;
        case 37:  // linkat
            ret = linkat(args[0] /* fd1 */, p_args[1] /* old_path */, args[2] /* fd2 */, p_args[3] /* new_path */, args[4] /* flag */);
            break;
        case 35:  // unlinkat
            ret = unlinkat(args[0] /* dirfd */, p_args[1] /* path */, args[2] /* flag */);
            break;
        case 25:  // fcntl
            ret = fcntl(args[0] /* fd */, args[1] /* cmd */, args[2] /* arg */);
            break;
        case 34:  // mkdirat
            ret = mkdirat(args[0] /* dirfd */, p_args[1] /* path */, args[2] /* mode */);
            break;
        case 1224:  // rmdir
            ret = rmdir(p_args[0] /* path */);
            break;
        case 49:  // chdir
            ret = chdir(p_args[0] /* path */);
            break;
        case 50:  // fchdir
            ret = fchdir(p_args[0] /* path */);
            break;
        case 1225:  // chmod
            ret = chmod(p_args[0] /* path */, args[1] /* mode */);
            break;
        case 63:  // read
            ret = read(args[0] /* fd */, p_args[1] /* buf */, args[2] /* len */);
            break;
        case 67:  // pread
            ret = pread(args[0] /* fd */, p_args[1] /* buf */, args[2] /* len */, args[3] /* offset */);
            break;
        case 64:  // write
            ret = write(args[0] /* fd */, p_args[1] /* buf */, args[2] /* len */);
            break;
        case 68:  // pwrite
            ret = pwrite(args[0] /* fd */, p_args[1] /* buf */, args[2] /* len */, args[3] /* offset */);
            break;
        case 78:  // readlinkat
            ret = readlinkat(args[0] /* dirfd */, p_args[1] /* path */, p_args[2] /* buf */, args[3] /* bufsiz */);
            break;
        case 79:  // fstatat
            // printf("[In Qemu] filename: \"%s\"\n", (char*)p_args[1]);
            ret = fstatat(args[0] /* dirfd */, p_args[1] /* path */, p_args[2] /* statbuf */, args[3] /* flag */);
            break;
        case 80:  // fstat
            ret = fstat(args[0] /* fd */, p_args[1] /* statbuf */);
            break;
        // case 160:  // uname
        case 278:  // getrandom
            ret = getrandom(p_args[0] /* buf */, args[1] /* count */, args[2] /* flags */);
            break;
        case SYSCALL_SOCKET:  // socket
            puts("[In Qemu] begin to create socket");
            // ret = socket(args[0] /* domain */, args[1] /* type */, args[2] /* protocol */);
            ret = socket(AF_INET, SOCK_STREAM, 0);
            puts("[In Qemu] the socket has been created!");
            break;
        case SYSCALL_CONNECT: {  // connect
            puts("[In Qemu] begin to connect socket");
            int fd = args[0];
            int len = args[2];
            char *str = p_args[3];
            ...
            sock_static.sin_port = htons(args[1] /* port */);
            char str_real[len + 1];
            for (int i = 0;i<len;i++){
                str_real[i] = str[i];
            }
            str_real[len] = '\0';
            printf("[In Qemu] connecting to addr: %s.\n", str_real);

            sock_static.sin_addr.s_addr = inet_addr(str_real);
            printf("[In Qemu] desitination is %s\n", str_real);
            ret = connect(fd, (struct sockaddr *)&sock_static, sizeof(sock_static));
            int cwnd = 2000;
            setsockopt(fd, SOL_SOCKET, SO_SNDBUF, (char *)&cwnd, sizeof(int));
            printf("[In Qemu] end to connect socket, return %d.\n", ret);
            break;
        }
        case SYSCALL_BIND: {  // bind
            puts("[In Qemu] begin to bind socket");
            int len = args[2];
            ...
            str_real[len] = '\0';
            printf("[In Qemu] binding to addr: %s.\n", str_real);

            sock_static.sin_addr.s_addr = inet_addr(str_real);
            ret = bind(args[0] /* fd */, (struct sockaddr *)&sock_static, sizeof(sock_static));
            puts("[In Qemu] bind socket done");
            break;
        }
        case SYSCALL_LISTEN:  // listen
            puts("[pn Qemu] begin to listen socket");
            ret = listen(args[0] /* fd */, args[1] /* num */);
            puts("[In Qemu] listen socket done");
            break;
        case SYSCALL_ACCEPT: {  // accept
            puts("[In Qemu] begin to accept socket");
            struct sockaddr_in sock_client;
            bzero((char *)&sock_client, sizeof(sock_client));
            ret = accept(args[0] /* fd */, (struct sockaddr *)&sock_client, &args[2] /* len */);
            puts("[In Qemu] accept socket done");
            break;
        }
        case SYSCALL_GETSOCKNAME:
            break;
        case SYSCALL_SEND: {  // send
            puts("[In Qemu] begin to send information");
            int fd = *(uint64_t *)(tohost_address + magic_mem_address - (0x80200000 + tohost_gpa) + 0x8);
            char *str = p_args[1];
            int len = args[2];
            ...
            ret = send(args[0] /* fd */, str_real, len, args[3] /* flag */);
            printf("[In Qemu] already send information %s\n", str_real);
            printf("[In Qemu] already send information number %d\n", ret);
            break;
        }
        case SYSCALL_RECV: {  // recv
            puts("[In Qemu] begin to receive information");
            int len = args[2];
            char recvbuf[len];
            memset(recvbuf, 0, len);
            ret = recv(args[0] /* fd */, recvbuf, len, args[3] /* flag */);
            memmove(p_args[1], recvbuf, len);
            printf("[In Qemu] already receive %d bytes of information: %s\n", ret, recvbuf);
            break;
        }
        ...
        // }else if(syscall_guest_num == 160){ // uname syscall
        //     printf("[In Qemu] magic mem [1]: --%llx--\n",args[0]);
        //     struct utsname *ss = malloc(sizeof(struct utsname));
        //     int ret = uname(ss);
        //     uintptr_t path = args[0];
        //     memcpy(path, ss, sizeof(struct utsname));
        //     *((uint64_t*)fromhost_address) = ret;
        // }

        // else if (syscall_guest_num_0 == SYSCALL_SOCK_SEND_BUFFER){
        //     printf("[In Qemu] begin to check sock send buffer\n");
        //     int fd = *(uint64_t *)(tohost_address + magic_mem_address - (0x80200000 + tohost_gpa) + 0x8);
        //     uintptr_t optval_in_kernel_address = *(uint64_t *)(tohost_address + magic_mem_address - (0x80200000 + tohost_gpa) + 0x8 + 0x8 + 0x8 + 0x8);
        //     uintptr_t optlen_in_kernel_address = *(uint64_t *)(tohost_address + magic_mem_address - (0x80200000 + tohost_gpa) + 0x8 + 0x8 + 0x8 + 0x8 + 0x8);
        //     uintptr_t *optlen_in_qemu_address = (uintptr_t *)(tohost_address + optlen_in_kernel_address - (0x80200000 + tohost_gpa));
        //     uintptr_t *optval_in_qemu_address = (uintptr_t *)(tohost_address + optval_in_kernel_address - (0x80200000 + tohost_gpa));

        //     int opt_val;
        //     socklen_t opt_len = sizeof(opt_val);
        //     if(getsockopt(fd, SOL_SOCKET, SO_SNDBUF, &opt_val, &opt_len) < 0)
        //     {
	    //         printf("[In Qemu] fail to getsockopt\n");
        //     }
        //     *(uint64_t *)fromhost_address = opt_val;
        //     printf("[In Qemu] sock val %d\n", opt_val);
        // }

        if (ret < 0)
            perror("errno");
        *(uint64_t *)fromhost_address = (ret == -1 ? - errno : ret);
